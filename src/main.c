...
int main(int argc, char **argv)
{
	...
	if (parseCommandline(&argc, &argv)) {
		if(initLockFile()) {
			printMessage(MSG_INFO, "Startup %s %s (-h to print usage) http://latte-desktop.org/ \n", OBS_NAME, OBS_VERSION);
			if (initConfigs()) {
				initWidgets();
...
void initWidgets(void)
{
	...
	gtk_widget_set_can_focus(winWidget, TRUE);

		gtk_window_set_type_hint(GTK_WINDOW(winWidget), GDK_WINDOW_TYPE_HINT_DOCK);
		gtk_window_set_keep_above(GTK_WINDOW(winWidget), TRUE);
